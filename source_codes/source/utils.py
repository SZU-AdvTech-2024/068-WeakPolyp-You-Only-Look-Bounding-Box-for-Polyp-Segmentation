...
def preprocess(path_src):
    ...
    path_dst = path_src.replace('/SUN-SEG/', '/../dataset/SUN-SEG-Processed_352_forzoom/')
    for folder in os.listdir(path_src+'/Frame'):
        ...
        for name in os.listdir(path_src+'/Frame/'+folder):
            ...
            # edge     = cv2.imread(path_src+'/Edge/'+folder+'/'+name.replace('.jpg', '.png'), cv2.IMREAD_GRAYSCALE)
            # edge     = cv2.resize(edge, (384, 384), interpolation=cv2.INTER_LINEAR)
            # _, edge = cv2.threshold(edge, 127, 255, cv2.THRESH_BINARY)
            contours, _ = cv2.findContours(mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
            edge = np.zeros_like(mask)
            # 绘制边界
            cv2.drawContours(edge, contours, -1, color=255, thickness=1)
            ...
            os.makedirs(path_dst + '/Edge/' + folder, exist_ok=True)
            cv2.imwrite(path_dst + '/Edge/' + folder + '/' + name.replace('.jpg', '.png'), edge)
