        ...
        for epoch in range(self.cfg.epoch):
            ...
            for i, (image, mask) in enumerate(self.loader):
                with torch.cuda.amp.autocast():
                    ...
                    # size1          = np.random.choice([384])
                    image1         = F.interpolate(image, size=384, mode='bilinear')
                    pred1          = self.model(image1)
                    # pred1          = F.interpolate(pred1, size=352, mode='bilinear')
                    ## pred 2
                    # size2          = np.random.choice([384])
                    image2         = F.interpolate(image, size=384, mode='bilinear')
    ...
    def evaluate(self, epoch):
        ...
        with torch.no_grad():
            ...
            for image, mask, name in tqdm(loader):
                ...
                # image_          = F.interpolate(image, size=384, mode='bilinear')
                B, H, W         = mask.shape
        ...
        if dice/cnt>self.max_dice:
            ...
            torch.save(self.model.state_dict(), self.cfg.backbone+'/model_384_1125.pth')
        self.model.train()
...
if __name__=='__main__':
    ...
    class Config:
        def __init__(self, backbone):
            ...
            self.train_image    = '../dataset/SUN-SEG-Processed_384/TrainDataset/Frame'
            self.train_mask     = '../dataset/SUN-SEG-Processed_384/TrainDataset/Box'
            ## set the path of testing dataset
            self.test_image     = ['../dataset/SUN-SEG-Processed_384/TestHardDataset/Seen/Frame', '../dataset/SUN-SEG-Processed_384/TestHardDataset/Unseen/Frame']
            self.test_mask      = ['../dataset/SUN-SEG-Processed_384/TestHardDataset/Seen/GT'   , '../dataset/SUN-SEG-Processed_384/TestHardDataset/Unseen/GT'   ]
            ## set the path of logging
            self.log_path       = self.backbone+'/log_384_1125'
